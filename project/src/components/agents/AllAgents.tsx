...
const AllAgents: React.FC = () => {
  const { agents, isDataLoading } = useData();
  const { t } = useLanguage();
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <div className="h-4 w-80 mx-auto mt-4 bg-gray-200 rounded animate-pulse" />
          </div>
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl shadow p-6 animate-pulse">
                <div className="h-56 w-full bg-gray-200 rounded-lg mb-4" />
  ...
  if (!isDataLoading && activeAgents.length === 0) {
    return (
      <section className="py-16 bg-gray-50">
